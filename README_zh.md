...
- [ESP32 小车 MQTT 控制协议](docs/mqtt/car-mqtt-control.md) - Cardputer ADV 车控 fork 与麦轮/蜘蛛的 EMQX 协议（独立于小智云）
...
- 按键：Fn+1 聊天 / Fn+2 启动器（数字 1 Car / 2 Spider / 3 IceBox / 8 Snake 等进入子页）；车控页 `;` `.` `,` `/` 前进/停/左/右；空调页 P/M/F/;/. 即时发红外（S 强制重发）

详见 [docs/mqtt/car-mqtt-control.md](docs/mqtt/car-mqtt-control.md) 与 [板型 README](main/boards/m5stack-cardputer-adv-car/README_zh.md)。
