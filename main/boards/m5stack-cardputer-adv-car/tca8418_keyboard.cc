...
void Tca8418Keyboard::UpdateModifierState(uint8_t row, uint8_t col, bool pressed) {
    // Fn key: row 2, col 0 (Cardputer ADV; not the Opt key)
    if (row == 2 && col == 0) {
        if (pressed) {
            modifier_mask_ |= KEY_MOD_FN;
        } else {
            modifier_mask_ &= ~KEY_MOD_FN;
        }
    }
    // Aa (Shift) key: row 2, col 1
    else if (row == 2 && col == 1) {
        if (pressed) {
        ...
        // Caps lock toggle: Fn + Aa (matches M5Cardputer library behavior)
        if (pressed && (modifier_mask_ & KEY_MOD_FN) != 0) {
...
KeyEvent Tca8418Keyboard::MapKeyEvent(uint8_t row, uint8_t col, bool pressed) {
    ...
    if (event.key_code == KC_FN || event.key_code == KC_LSHIFT ||
        event.key_code == KC_LCTRL || event.key_code == KC_LALT ||
        event.key_code == KC_LOPT) {
