...
public:
     ...
     * device state. Call from ScheduleChatAudioRestore after Chat is visible —
     * never from Radio/Music OnLeave or boot Initialize (codec_ still null).
     */
...
private:
    ...
    std::atomic<int64_t> chassis_command_pending_until_us_{0};
    std::atomic<int64_t> suppress_server_audio_until_us_{0};
    std::string pending_chassis_serial_command_;
    std::string pending_chassis_reply_;
    int clock_ticks_ = 0;
