...
|------|------|-----------------|
| **Fn+1** | 聊天页 | （不发车控 MQTT） |
| **Fn+2** | 启动器 | （数字 **1** Car / **2** Spider / **3** IceBox 等进入子页） |
...
- 车控键在麦轮 / 蜘蛛页生效，**无需按 Fn**；Fn 仅用于 **Fn+1** 聊天、**Fn+2** 启动器；麦轮/蜘蛛/空调从启动器数字进入。
...
- 页面与按键：[`docs/architecture/pages-and-keys.md`](../architecture/pages-and-keys.md)
